    ...
    dispatch(register({ email: data.email, password: data.password }))
      ...
      .then(() => {
        toast({
          ...
          description: "You can now log in to your Eventory workspace.",
        });
        navigate("/login");
      })
      .catch((err) => {
        toast({
          ...
          description:
            err || "Something went wrong while creating your account.",
        });
      });
  };
      ...
      <div className="mx-auto grid w-full max-w-5xl gap-10 py-10 md:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] items-center">
        ...
        <div className="space-y-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card/60 px-3 py-1 text-xs font-medium text-muted-foreground backdrop-blur">
            ...
            <span>Host & discover events with ease</span>
          </div>

          <div className="space-y-2">
            <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">
              Create your Eventory account
            </h1>
            <p className="text-sm sm:text-base text-muted-foreground max-w-md">
              Join organizers and attendees using Eventory to run smooth, modern
              events — from intimate meetups to large conferences.
            </p>
          </div>
          ...
          <ul className="grid gap-3 text-sm text-muted-foreground sm:grid-cols-2 max-w-lg">
            <li className="flex items-start gap-2">
              ...
              <span>Manage attendees and registrations in one place.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-[11px] text-primary">
                ✓
              </span>
              <span>Launch events in minutes with a simple workflow.</span>
            </li>
          </ul>
        </div>
